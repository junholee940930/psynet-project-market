...
// "구인 중" = 형성/개발 단계(기획·예정·개발)라 협업자 합류가 가능한 프로젝트.
// 상용(라이브)·보류는 제외. max_participants는 대부분 기본값(10)이라 신뢰도 낮아 상태 기반으로 판단.
const RECRUITING_STATUS = new Set(["기획", "예정", "개발"]);

export async function GET() {
  const projects = listProjects().filter((p) => RECRUITING_STATUS.has(p.status));
